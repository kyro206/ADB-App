...
export function WirelessDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return <AppModal open={open} onClose={onClose} title="Conectar dispositivo" subtitle="Conecta o empareja mediante Depuración inalámbrica.">
    <md-tabs className="wireless-tabs">
      <md-primary-tab active={mode === 'connect' || undefined} onClick={() => setMode('connect')}>Conectar</md-primary-tab>
      <md-primary-tab active={mode === 'pair' || undefined} onClick={() => setMode('pair')}>Código</md-primary-tab>
      <md-primary-tab active={mode === 'qr' || undefined} onClick={() => setMode('qr')}>QR</md-primary-tab>
    </md-tabs>
    ...
    {mode === 'qr' && <section className="wireless-qr">
      <div className="wireless-qr__preview">
        {qrPayload ? <img src={qrPayload.qr_data_url} alt="Código QR para emparejar ADB" /> : <MaterialIcon name="qr_code_2" />}
      </div>
      <div>
        ...
        <div className="wireless-qr__actions">
          ...
          <md-filled-button disabled={!qrPayload || busy || undefined} onClick={() => qrPayload && run('pair_wireless_qr', { serviceName: qrPayload.service_name, password: qrPayload.password }, 'Esperando al dispositivo...', 'Dispositivo emparejado')}>Emparejar</md-filled-button>
        </div>
      </div>
    </section>}
