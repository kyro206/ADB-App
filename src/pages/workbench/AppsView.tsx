    ...
    <div className={`apps-material-page ${selectedPackage ? 'detail-open' : ''}`}>
      <section className="apps-material-catalog">
        <header className="apps-material-toolbar">
          <md-outlined-text-field className="apps-material-search" label="Buscar por nombre o paquete" value={filter} onInput={(event: Event) => onFilterChange((event.target as HTMLInputElement).value)}>
        ...
        <div className="apps-material-grid">
          {filteredApps.map(app => <button className={`apps-material-tile ${selectedPackage === app.package_name ? 'selected' : ''}`} key={app.package_name} onClick={() => onSelect(app)}>
            <div className={`apps-material-status-icon ${app.disabled ? 'disabled' : ''}`} title={app.disabled ? 'Deshabilitada' : app.system_app ? 'Sistema' : 'Usuario'}>
        ...
        {!details ? <div className="apps-material-empty detail"><MaterialIcon name="touch_app" /><strong>Selecciona una aplicación</strong><span>Consulta detalles, acciones, energía y permisos.</span></div> : <>
          ...
          <header className="apps-material-detail__hero">
            ...
            <div>
              ...
              <div className={`apps-material-status-icon ${details.disabled ? 'disabled' : ''}`} title={details.disabled ? 'Deshabilitada' : details.system_app ? 'Aplicación del sistema' : 'Aplicación de usuario'}>
          ...
          <section className="apps-material-section">
            <header><span className="apps-material-section__title"><MaterialIcon name="bolt" /><h3>Acciones</h3></span></header>
          ...
          {detailsLoading ? (
            ...
            <>
              <section className="apps-material-section">
                <header><span className="apps-material-section__title"><MaterialIcon name="battery_android_frame_full" /><h3>Uso en segundo plano</h3></span></header>
                <div className="apps-material-energy">{([['unrestricted','Sin restricciones','speed'],['optimized','Optimizada','eco'],['restricted','Restringida','battery_saver']] as const).map(([value,label,icon])=><button key={value} className={details.background_mode===value?'active':''} onClick={()=>onBackgroundMode(value)}><MaterialIcon name={icon}/><span><strong>{label}</strong><small>{value==='unrestricted'?'Permite actividad continua':value==='optimized'?'Android administra el consumo':'Limita la actividad de fondo'}</small></span><md-ripple/></button>)}</div>
              </section>

              <section className="apps-material-section">
                <header><span className="apps-material-section__title"><MaterialIcon name="info" /><h3>Información</h3></span></header>
              ...
              <section className="apps-material-section">
                <header>
                  <span className="apps-material-section__title"><MaterialIcon name="shield" /><h3>Permisos</h3></span>
                  <span className="apps-material-section__meta"><span>{details.permissions.filter(permission=>permission.granted).length} de {details.permissions.length}</span></span>
                </header>
